...
impl FromStr for CoinHash {
    ...
    fn from_str(
        ...
        for (index, byte) in bytes.iter_mut().enumerate() {
            ...
            let high =
                hex_nibble(encoded[offset])
                    .ok_or(CoinHashParseError)?;

            let low =
                hex_nibble(encoded[offset + 1])
                    .ok_or(CoinHashParseError)?;
...
mod tests {
    ...
    fn output_and_change_are_domain_separated() {
        ...
        let output =
            CoinHash::from_output(&commitment, 0);

        let change =
            CoinHash::from_change(&commitment, 0);
    ...
    fn output_index_changes_coin_hash() {
        ...
        let second =
            CoinHash::from_output(&commitment, 1);
