use paqus::crypto::{
    ...
    verify, verify_batch_parallel, verify_dual_parallel,
};
use sqisign_rs::{Level5, PublicKey as RawPublicKey};
use std::hint::black_box;
use std::time::{Duration, Instant};

const MESSAGE: &[u8] = b"Paqus SQIsign Level 5 verifier benchmark";
...
fn main() {
    ...
    let batch = measured(iterations, || {
        let results = verify_batch_parallel(black_box(&block_jobs));
