use paqus::block::MAX_BLOCK_WEIGHT;
use paqus::codec::{canonical_bytes, canonical_deserialize};
use paqus::consensus::supply::{Amount, BASE_BLOCK_REWARD};
use paqus::consensus::{
    DIFFICULTY_START, WBDA_WINDOW, next_difficulty_from_window, next_reward_from_window,
};
use paqus::crypto::{Address, address_from_string, address_to_string, argon2id_proof_of_work_hash};
use paqus::genesis::{decode_genesis_paqus, genesis_paqus_bytes};
use paqus::qcash::{
...
fn main() {
    let iterations = std::env::var("PAQUS_CORE_BENCH_ITERATIONS")
        .ok()
        .and_then(|value| value.parse::<u32>().ok())
        .filter(|value| *value > 0)
        .unwrap_or(25);

    println!("Paqus core benchmark ({iterations} measured iterations)");
    ...
    let genesis = genesis_paqus_bytes().expect("canonical genesis artifact");
    report(
        "genesis artifact validation",
        measure(iterations.min(5), || {
            black_box(decode_genesis_paqus(black_box(&genesis))).unwrap();
        }),
    );
    ...
    report(
        ...
        measure(pow_iterations, || {
            black_box(argon2id_proof_of_work_hash(black_box(
                b"paqus-core-benchmark-header",
            )))
            .unwrap();
        }),
    );
