use crate::block::{Block, BlockHeight, Header, MAX_BLOCK_WEIGHT};
...
pub fn transaction_bytes(transaction: &Transfer) -> Result<Vec<u8>, CodecError> {
    canonical_bytes(transaction)
}

pub fn signed_transaction_bytes(transaction: &SignedTransfer) -> Result<Vec<u8>, CodecError> {
    canonical_bytes(transaction)
}
...
pub fn transaction_hash(transaction: &Transfer) -> Result<TransactionHash, CodecError> {
    Ok(TransactionHash(
...
pub fn decode_transaction(bytes: &[u8]) -> Result<Transfer, CodecError> {
    if bytes.len() > crate::transaction::MAX_TX_SIZE {
        return Err(CodecError::InvalidTransaction);
    }
    let transaction: Transfer = canonical_deserialize(bytes)?;
    transaction
        .validate()
...
pub fn decode_signed_transaction(bytes: &[u8]) -> Result<SignedTransfer, CodecError> {
    if bytes.len() > crate::transaction::MAX_TX_SIZE {
        return Err(CodecError::InvalidTransaction);
    }
    let transaction: SignedTransfer = canonical_deserialize(bytes)?;
    transaction
        .validate_signed()
        .map_err(|_| CodecError::InvalidTransaction)?;
    Ok(transaction)
...
#[cfg(feature = "devnet")]
pub fn decode_signed_agile_transfer(
    bytes: &[u8],
    height: crate::block::BlockHeight,
    upgrade: Option<crate::crypto::CryptoUpgradePlan>,
) -> Result<crate::transaction::SignedAgileTransfer, CodecError> {
    if bytes.len() > crate::transaction::MAX_TX_SIZE {
        return Err(CodecError::InvalidTransaction);
    }
    let transaction: crate::transaction::SignedAgileTransfer = canonical_deserialize(bytes)?;
    transaction
...
where
    F: FnOnce(crate::crypto::Address) -> Option<crate::state::AccountAuthorization>,
{
    ...
    let account = crate::state::Account {
        address: signer,
        authorization: policy_for(signer),
    };
    transaction
...
/// choice, or Emission subsidy against a ledger.
...
mod tests {
    ...
    fn protocol_decoder_uses_stored_account_authorization() {
        ...
        assert!(
            decode_signed_protocol_transaction_at(&bytes, crate::block::Height(1), |_| None)
                .is_err()
        );
    }
}
