...
mint authority. Every asset exposes its current `mint_nonce` and cumulative
`total_minted`; each Mint must be signed by the mint authority, use exactly
`mint_nonce + 1`, and keep `total_minted` within `max_supply`.
