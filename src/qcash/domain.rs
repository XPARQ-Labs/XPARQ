...
pub struct QCashWithdrawalMetadata {
...
/// The wallet-held redeem secret is deliberately absent.
...
pub struct QCashRedeemMetadata {
    pub inputs: Vec<QCashRedeemInput>,
}
...
pub enum QCashError {
    ...
    InvalidCommitment,
    InvalidRedeemAuthorization,
    InvalidQCashFile,
    QCashFileTooLarge,
    TooManyWithdrawalOutputs,
    TooManyRedeemInputs,
    Serialization,
}
...
impl fmt::Display for QCashError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::EmptyRedeemInputs => f.write_str("QCash redeem must contain at least one input"),
            Self::DuplicateRedeemInput => {
                f.write_str("QCash redeem contains a duplicate coin reference")
            }
            Self::InvalidCommitment => {
                f.write_str("QCash coin redeem key does not match commitment")
            }
            Self::InvalidRedeemAuthorization => {
                f.write_str("QCash redeem authorization is invalid for its recipient")
            }
            ...
            Self::TooManyWithdrawalOutputs => {
                f.write_str("withdraw creates too many QCash outputs")
            }
            Self::TooManyRedeemInputs => f.write_str("QCash redeem contains too many inputs"),
            Self::Serialization => f.write_str("failed to serialize QCash data"),
        }
    }
}

pub fn qcash_redeem_key_commitment_from_secret(redeem_secret: &[u8; 32]) -> [u8; 32] {
    qcash_redeem_key_commitment(&public_key_from_seed(redeem_secret))
}

pub fn qcash_redeem_key_commitment(public_key: &PublicKey) -> [u8; 32] {
    domain_hash(HashDomain::QCashRedeemKeyCommitment, &public_key.0).0
}
...
fn redeem_authorization_bytes(
    ...
    struct RedeemAuthorizationPayload {
        chain_id: u32,
        protocol_version: u8,
        operation: u8,
        coin_id: [u8; 32],
    ...
    let payload = RedeemAuthorizationPayload {
    ...
    Ok(domain_hash(
        HashDomain::QCashRedeemAuthorization,
...
/// Encodes one bearer coin using the only supported `.QCash` binary format.
...
/// Strictly decodes and checks a canonical `.QCash` bearer coin file.
...
impl QCashCoinFile {
    ...
    pub fn redeem_input_for_transaction(
        &self,
        recipient: Address,
        transaction_commitment: [u8; 32],
    ) -> Result<QCashRedeemInput, QCashError> {
    ...
    pub fn redeem_key_commitment(&self) -> [u8; 32] {
        qcash_redeem_key_commitment_from_secret(&self.redeem_secret)
    }
}

impl QCashRedeemMetadata {
    pub fn from_inputs(inputs: Vec<QCashRedeemInput>) -> Result<Self, QCashError> {
    ...
    pub fn new_for_transaction(
        ...
        let inputs = files
            .iter()
            .map(|file| file.redeem_input_for_transaction(recipient, transaction_commitment))
    ...
    pub fn validate_authorizations_for_transaction(
        ...
        for input in &self.inputs {
            ...
            if !verify(&input.redeem_public_key, &message, &input.authorization) {
                return Err(QCashError::InvalidRedeemAuthorization);
            }
        }
        Ok(())
    }
    ...
    pub fn validate(&self) -> Result<(), QCashError> {
        ...
        if self.inputs.is_empty() {
            return Err(QCashError::EmptyRedeemInputs);
        }
        if self.inputs.len() > MAX_QCASH_REDEEM_INPUTS {
            return Err(QCashError::TooManyRedeemInputs);
        }
        ...
        for input in &self.inputs {
            ...
            if !references.insert(input.coin_id) {
                return Err(QCashError::DuplicateRedeemInput);
            }
        }
        self.amount().map(|_| ())
    }
...
impl QCashRedeemInput {
    ...
    pub fn redeem_key_commitment(&self) -> [u8; 32] {
        qcash_redeem_key_commitment(&self.redeem_public_key)
    }
}
...
impl QCashWithdrawalMetadata {
    ...
    pub fn validate(&self) -> Result<(), QCashError> {
        ...
        if self.outputs.len() > MAX_QCASH_WITHDRAWAL_OUTPUTS {
            return Err(QCashError::TooManyWithdrawalOutputs);
        }
