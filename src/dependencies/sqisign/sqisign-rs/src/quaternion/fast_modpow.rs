...
fn modpow_uint<const N: usize>(
    base: &Uint<N>,
    exp: &Uint<N>,
    params: &FixedMontyParams<N>,
) -> Uint<N> {
    let base_monty = FixedMontyForm::new(base, params);
...
fn ibz_probab_prime_uint<const N: usize>(n_abs: &BigInt, n_bytes: &[u8], reps: u32) -> i32 {
    ...
    let params = FixedMontyParams::<N>::new_vartime(n_odd);
...
fn miller_rabin_uint<const N: usize>(
    ...
    let one = FixedMontyForm::one(params);
    let neg_one = FixedMontyForm::new(nm1, params);
    let mut y = FixedMontyForm::new(a, params).pow(q);
...
fn strong_lucas_uint<const N: usize>(n: &Uint<N>, params: &FixedMontyParams<N>, bits: u32) -> bool {
    ...
    let zero = FixedMontyForm::new(&Uint::<N>::ZERO, params);
    let two = FixedMontyForm::new(&Uint::<N>::from_u8(2), params);
...
fn lucas_mod_uint<const N: usize>(
    ...
    b0: u32,
    bits: u32,
) -> (bool, FixedMontyForm<N>, FixedMontyForm<N>) {
    let one = FixedMontyForm::one(params);
    let zero_mf = FixedMontyForm::new(&Uint::<N>::ZERO, params);

    let q_abs = q_val.unsigned_abs();
    let q_u = Uint::<N>::from_u64(q_abs);
    let q_monty = if q_val >= 0 {
        FixedMontyForm::new(&q_u, params)
    } else {
        let n_minus_q = n.wrapping_sub(&q_u);
        FixedMontyForm::new(&n_minus_q, params)
    };
    ...
    while bs >= b0 as i32 {
        ...
        let do_step = bs == b0 as i32 || bool::from(n.bit(bs as u32));
        if do_step {
            qk *= q_monty;
            ...
            let neg_2q = if q_val >= 0 {
                ...
                FixedMontyForm::new(&n_minus_val, params)
            } else {
                FixedMontyForm::new(&Uint::<N>::from_u64(2 * q_abs), params)
            };
            v = u + neg_2q * old_u;
        }
...
fn half_monty<const N: usize>(
    val: FixedMontyForm<N>,
    n: &Uint<N>,
    params: &FixedMontyParams<N>,
) -> FixedMontyForm<N> {
    let v = val.retrieve();
    let result = if bool::from(v.bit(0)) {
        v.wrapping_add(n).shr(1)
    } else {
        v.shr(1)
    };
    FixedMontyForm::new(&result, params)
}
...
fn ibz_sqrt_mod_p_uint<const N: usize>(amod: &Ibz, p: &Ibz) -> Option<Ibz> {
    ...
    let params = FixedMontyParams::<N>::new_vartime(p_odd);
    ...
    if p_mod_4 == 3 {
        ...
        if test == Uint::<N>::ONE {
            ...
            let a_monty = FixedMontyForm::new(&a_u, &params);
            let four = Uint::<N>::from_u8(4);
            let a4_u = {
                let a4_monty = FixedMontyForm::new(&four, &params) * a_monty;
                a4_monty.retrieve()
            };
            let tmp = modpow_uint(&a4_u, &exp, &params);
            let tmp_monty = FixedMontyForm::new(&tmp, &params);
            let two = Uint::<N>::from_u8(2);
            let a2_monty = FixedMontyForm::new(&two, &params) * a_monty;
        ...
        let mut z_monty = FixedMontyForm::new(&qnr, &params).pow(&q);
        let mut y_monty = FixedMontyForm::new(&a_u, &params).pow(&q);
        let q_plus_1_half = q.wrapping_add(&Uint::<N>::ONE).shr(1);
        let mut x_monty = FixedMontyForm::new(&a_u, &params).pow(&q_plus_1_half);

        let neg_one_monty = FixedMontyForm::new(&pm1, &params);
...
pub fn ibz_legendre(a: &Ibz, p: &Ibz) -> i32 {
    ...
    if fits_in::<8>(p) {
        ...
        let params = FixedMontyParams::<8>::new_vartime(p_odd);
        ...
        // p is an odd prime (precondition).
        let p_odd = Odd::new(p_u).expect("invariant: p is an odd prime");
        let params = FixedMontyParams::<16>::new_vartime(p_odd);
...
mod tests {
    ...
    fn test_strong_lucas() {
        for &p in &[5u64, 7, 11, 13, 17, 101, 1009, 10007] {
            ...
            let params = FixedMontyParams::<8>::new_vartime(n_odd);
            assert!(
