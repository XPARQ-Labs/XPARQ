# SQIsign improvement plan and changelog

## Status

SQIsign is a NIST Round 3 candidate. The `paqus-sqisign` 0.1.0 crate is based
on the vendored `sqisign-rs` 0.5.0
implementation is not the official NIST reference implementation and states
that it has not been independently audited. Nothing in this document activates
SQIsign in Paqus consensus.

Baseline:

- `paqus-sqisign`: 0.1.0 (vendored `sqisign-rs` 0.5.0 baseline)
- `sqisign-verify`: 0.5.0
- security level prepared by Paqus: Level 5
- Paqus registry status: `CandidateInactive`
...
1. Default Paqus build without the candidate feature.
...
8. No change to Paqus consensus or wire encoding while the candidate is
   inactive.
...
- Replace serialized candidate secret `Vec<u8>` storage with a fixed-size,
  zeroized allocation.
- Prevent secret cloning and redact all formatting.
- [x] Contain panics reachable through the Paqus candidate keygen/sign adapter
...
- Add parser fuzzing for public keys, signatures, and signing keys.
- Add differential fuzzing against the official reference implementation.
- Add verification work accounting before any consensus activation.
- Measure signing timing leakage on supported production targets.

## RNG/signature migration validation

Completed:

- Paqus default and `sqisign-candidate` builds.
...
- Paqus candidate integration-test and benchmark compilation.
...
Compatible dependencies now use explicit shared Paqus paths, including when
`paqus-sqisign` is built as a standalone crate:
...
- `crypto-bigint` 0.7.5, `num-bigint` 0.5.1, and `num-integer` 0.1.46:
  Paqus has no other matching local implementation.
...
- `paste` 1.0.15 and `libm` 0.2.16: Paqus has no duplicate matching copy.
...
- Added signing and verification transcript boundary modules and moved the
  SQIsign production graph from `sha3` 0.10.9/`keccak` 0.1.6 to Paqus-local
...
- Moved `sha3` 0.10.9 to a test-only `sha3_legacy` oracle and removed the old
  SHA3/Keccak patches from the Paqus production manifest.
...
- Ported the SQIsign benchmark from removed `rand::thread_rng()` to
  `rand::rng()`.
- Confirmed the Paqus candidate build, Criterion signing benchmark, SHA3,
...
- The legacy SHA3 backend remains a development oracle in addition to the
  ...
  are still absent.
- Redirected compatible SQIsign dependencies to explicit shared Paqus paths
...
## 2026-07-29 — Paqus verifier hot-path
...
- SQIsign blockchain builds now install the crate's zeroizing global allocator
  from Paqus core. Heap blocks used by bigint/arithmetic intermediates are
