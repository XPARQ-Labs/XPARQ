...
impl CryptoPrimitive {
    pub const fn family(self) -> CryptoPrimitiveFamily {
        match self {
            ...
            Self::ProofOfWork(_) => CryptoPrimitiveFamily::ProofOfWork,
        }
    }
}
...
pub struct CryptoUpgradePlan {
    ...
    /// Protocol version required at and after `transition_height`.
...
impl CryptoUpgradePlan {
    pub fn validate(self, current_protocol_version: u8) -> Result<(), CryptoUpgradeError> {
        ...
        if self.protocol_version <= current_protocol_version {
            return Err(CryptoUpgradeError::ProtocolVersionNotIncreased);
        }
        Ok(())
    }
