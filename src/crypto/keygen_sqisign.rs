...
type PaqusSignature = SqisignSignature<Level5>;
...
pub struct CachedVerifyingKey {
    inner: Option<Arc<PaqusVerifyingKey>>,
}
...
fn keypair_from_parts(public_key: PaqusVerifyingKey, signing_key: PaqusSigningKey) -> KeyPair {
...
pub fn authorization_seed_from_password(
    ...
    let mut salt_material =
        Vec::with_capacity(b"PAQUS_SQISIGN_LEVEL5_AUTHORIZATION_V1".len() + 4 + PUBLIC_KEY_SIZE);
    salt_material.extend_from_slice(b"PAQUS_SQISIGN_LEVEL5_AUTHORIZATION_V1");
...
pub fn cached_verifying_key(public_key: &PublicKey) -> CachedVerifyingKey {
    ...
    let Ok(decoded) = PaqusVerifyingKey::from_bytes(&public_key.0) else {
...
impl CachedVerifyingKey {
    pub fn verify(&self, message: &[u8], signature: &Signature) -> Result<(), CryptoError> {
        ...
        let signature = PaqusSignature::from_bytes(&signature.0)
...
struct VerifyingKeyCache {
    entries: HashMap<PublicKey, Arc<PaqusVerifyingKey>>,
...
impl VerifyingKeyCache {
    ...
    fn get(&self, key: &PublicKey) -> Option<Arc<PaqusVerifyingKey>> {
    ...
    fn insert(
        &mut self,
        key: PublicKey,
        decoded: Arc<PaqusVerifyingKey>,
    ) -> Arc<PaqusVerifyingKey> {
        ...
        .get_or_init(|| {
            ...
            for index in 0..worker_count {
                ...
                if std::thread::Builder::new()
                    .name(format!("paqus-sqisign-l5-{index}"))
                    .spawn(move || {
                        loop {
                            let job = {
