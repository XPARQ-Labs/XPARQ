...
pub(crate) const ASSET_HASH_SIZE: usize = blake3::OUT_LEN;
pub(crate) const ASSET_HASH_PREFIX: &str = "asset:";
const ASSET_HASH_CONTEXT: &str = "xparq:asset-id:v1";

/// Derives an asset identifier from unambiguous length-delimited fields.
pub(crate) fn derive(fields: &[&[u8]]) -> [u8; ASSET_HASH_SIZE] {
    let mut hasher = blake3::Hasher::new_derive_key(ASSET_HASH_CONTEXT);
    for field in fields {
...
pub(crate) fn fmt(
    ...
    formatter.write_str(ASSET_HASH_PREFIX)?;
    for byte in bytes {
...
pub(crate) fn parse(value: &str) -> Result<[u8; ASSET_HASH_SIZE], AssetHashParseError> {
    let encoded = value
        .strip_prefix(ASSET_HASH_PREFIX)
        .ok_or(AssetHashParseError)?;
...
mod tests {
    ...
    fn text_format_is_explicit_and_canonical() {
        ...
        assert!(parse(&"ab".repeat(ASSET_HASH_SIZE)).is_err());
        assert!(parse(&format!("ASSET:{}", "ab".repeat(ASSET_HASH_SIZE))).is_err());
        assert!(
            parse(&format!(
                "{ASSET_HASH_PREFIX}{}",
                "AB".repeat(ASSET_HASH_SIZE)
            ))
            .is_err()
        );
    }
