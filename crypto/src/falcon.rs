...
impl FalconPublicKey {
    pub fn from_bytes(level: FalconLevel, bytes: Vec<u8>) -> Result<Self, FalconError> {
...
pub fn generate_keypair(level: FalconLevel) -> Result<FalconKeyPair, FalconError> {
...
fn generate_keypair_with_rng(
    ...
    Ok(FalconKeyPair {
        public_key: FalconPublicKey::from_bytes(level, public)
            .map_err(|_| FalconError::KeyGenerationFailed)?,
...
pub fn derive_public_key(secret_key: &FalconSecretKey) -> Result<FalconPublicKey, FalconError> {
...
pub fn sign(secret_key: &FalconSecretKey, message: &[u8]) -> Result<FalconSignature, FalconError> {
    let level = secret_key.level;
    ...
    result.ok_or(FalconError::SigningFailed)?;
