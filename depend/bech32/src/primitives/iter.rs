        ...
        if let Some(last) = self.last_byte {
            match bit_offset.cmp(&3) {
                ...
                Greater => {
                    ...
                    Some(Fe32(
                        ((last << (bit_offset - 3)) | (next >> (11 - bit_offset))) & 0x1f,
                    ))
                }
            }
        } else {
...
// If the total number of bits is not a multiple of 8, any trailing bits are dropped.
fn fes_len_to_bytes_len(n: usize) -> usize {
    n * 5 / 8
}
        ...
        match self.iter.next() {
            ...
            None => {
                if self.checksum_remaining == 0 {
                    ...
                    Some(Fe32(
                        self.checksum_engine
                            .residue()
                            .unpack(self.checksum_remaining),
                    ))
                }
            }
        }
...
mod tests {
    ...
    fn bytes_to_fes_size_hint() {
        ...
        assert_eq!(
            DATA.iter().copied().bytes_to_fes().size_hint(),
    ...
    fn fe32_iter_ext_zeropad_and_nozeropad() {
        use std::convert::TryFrom;
        {
            ...
            let fes_iter = [0, 1, 2, 1]
                .iter()
                .copied()
                .map(|b| Fe32::try_from(b).unwrap());
            assert_eq!(
        ...
        }
        {
            ...
            assert_eq!(
                fes_iter.clone().fes_to_bytes().collect::<Vec<_>>(),
                [0, 68, 63]
            );
        }
        {
