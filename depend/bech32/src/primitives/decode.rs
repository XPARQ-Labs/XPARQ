...
impl<'s> UncheckedHrpstring<'s> {
    ...
    pub fn validate_checksum<Ck: Checksum>(&self) -> Result<(), ChecksumError> {
        ...
        if residue != Ck::TARGET_RESIDUE {
            return Err(InvalidResidue(InvalidResidueError::new(
                residue,
                Ck::TARGET_RESIDUE,
            )));
        }
...
impl Iterator for AsciiToFe32Iter<'_> {
    ...
    fn next(&mut self) -> Option<Fe32> {
        self.iter.next().map(Fe32::from_char_unchecked)
    }
    #[inline]
    fn size_hint(&self) -> (usize, Option<usize>) {
...
impl InvalidResidueError {
    ...
    /// on the FieldVecs.
    pub(super) fn residue(&self) -> Polynomial<Fe32> {
        self.actual.clone() - &self.target
    }
}
...
mod tests {
    ...
    fn bip_350_invalid_because_of_invalid_checksum() {
        ...
        for s in invalid {
            let err = UncheckedHrpstring::new(s)
                .unwrap()
    ...
    fn witness_version_enforces_bounds() {
        ...
        let checked_empty = CheckedHrpstring {
            hrp: Hrp::parse_unchecked("bc"),
            ascii: b"",
            hrpstring_length: 3,
        };
        ...
        let checked_valid = CheckedHrpstring {
            hrp: Hrp::parse_unchecked("bc"),
            ascii: b"s",
            hrpstring_length: 4,
        };
        ...
        let checked_too_large = CheckedHrpstring {
            hrp: Hrp::parse_unchecked("bc"),
            ascii: b"3",
            hrpstring_length: 4,
        };
    ...
    fn checksum_validation() {
        ...
        assert_eq!(
            at_limit.validate_checksum::<Bech32>(),
    ...
    fn validate_segwit_padding() {
        ...
        assert_eq!(
            checked(b"qqqqp").validate_segwit_padding(),
            Err(PaddingError::NonZero)
        );

        // padding_len = 2
        assert_eq!(
            checked(b"qp").validate_segwit_padding(),
            Err(PaddingError::NonZero)
        );
        assert_eq!(
            checked(b"qqqqqqp").validate_segwit_padding(),
            Err(PaddingError::NonZero)
        );
    }
    ...
    fn iterator_size_hints() {
        let mut fe_iter = AsciiToFe32Iter {
            iter: b"qpz".iter().copied(),
        };
