    ...
    pub fn bch_errors(&self) -> Option<ErrorIterator<'_, Ck>> {
        ...
        for loc in &self.erasures {
            let factor: Polynomial<_> = [
                Ck::CorrectionField::ONE,
                -Ck::ROOT_GENERATOR.powi(*loc as i64),
            ]
            .iter()
            .cloned()
            .collect(); // alpha^-ix - 1
...
mod tests {
    ...
    fn residue_error() {
        ...
        let residue = checksum_error
            .residue_error()
            .expect("checksum error should expose invalid residue");
        assert!(
            residue.residue_error().is_some(),
            "invalid residue error should expose itself"
        );
        ...
        assert!(
            segwit_no_data.residue_error().is_none(),
            "no-data errors are not correctable"
        );
