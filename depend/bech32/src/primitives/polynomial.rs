...
impl<F: Field> Polynomial<F> {
    ...
    pub fn into_inner(self) -> FieldVec<F> {
        self.inner
    }

    /// Provides access to the underlying [`FieldVec`].
    pub fn as_inner(&self) -> &FieldVec<F> {
        &self.inner
    }
    ...
    pub fn zero_is_root(&self) -> bool {
        self.inner.is_empty() || self.leading_term() == F::ZERO
    }
    ...
    pub fn convolution(&self, syndromes: &Self) -> Self {
        ...
        for idx in 0..terms {
            ret.push(
                (0..n)
                    .map(|i| self.inner[n - i - 1].clone() * &syndromes.inner[idx + i])
                    .sum(),
            );
        }
        Self::from(ret)
    ...
    pub fn mul_mod_x_d(&self, other: &Self, d: usize) -> Self {
        ...
        let convolution_product = |exp: usize| {
            ...
            (sidx..=eidx)
                .map(|i| self.inner[exp - i].clone() * &other.inner[i])
                .sum()
        };
...
impl<F: Field> iter::FromIterator<F> for Polynomial<F> {
    ...
    {
        Polynomial {
            inner: FieldVec::from_iter(iter),
        }
    }
}

impl<F> From<FieldVec<F>> for Polynomial<F> {
    fn from(inner: FieldVec<F>) -> Self {
        Self { inner }
    }
}
...
mod tests {
    ...
    fn mul_mod() {
        ...
        assert_eq!(
            x_minus_1.mul_mod_x_d(&x_minus_1, 2),
            [Fe32::P].iter().copied().collect(),
        );
    }
    ...
    fn factor_then_mul() {
        ...
        let bech32_poly_lift = Polynomial {
            inner: bech32_poly.inner.lift(),
        };
