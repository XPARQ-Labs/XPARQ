...
impl<F> FieldVec<F> {
    ...
    pub fn has_data(&self) -> bool {
        self.len <= NO_ALLOC_MAX_LENGTH || cfg!(feature = "alloc")
    }
...
impl<F: Field> FieldVec<F> {
    ...
    pub fn from_powers(elem: F, n: usize) -> Self {
        iter::successors(Some(F::ONE), |gen| Some(elem.clone() * gen))
            .take(n + 1)
            .collect()
    }
...
impl<F: Default> FieldVec<F> {
    ...
    pub fn push_back(&mut self, item: F) {
        self.push(item)
    }
...
mod tests {
    ...
    fn iter_slice() {
        ...
        assert!(x[..NO_ALLOC_MAX_LENGTH]
            .iter()
            .copied()
            .eq(0..NO_ALLOC_MAX_LENGTH));
        assert!(x[1..].iter().copied().eq(1..NO_ALLOC_MAX_LENGTH));
        assert!(x[..NO_ALLOC_MAX_LENGTH - 1]
            .iter()
            .copied()
            .eq(0..NO_ALLOC_MAX_LENGTH - 1));
        assert!(x[1..NO_ALLOC_MAX_LENGTH - 1]
            .iter()
            .copied()
            .eq(1..NO_ALLOC_MAX_LENGTH - 1));
    ...
    fn field_ops() {
        ...
        assert!(ps
            .iter()
            .copied()
            .eq(iter::repeat(Fe32::P).take(NO_ALLOC_MAX_LENGTH)));
        assert_eq!(
            pzr.iter().copied().collect::<Vec<_>>(),
            [Fe32::P, Fe32::Z, Fe32::Y, Fe32::G,]
        );

        let pow2 = pzr.clone().mul_pointwise(&pzr);
        assert_eq!(
            pow2.iter().copied().collect::<Vec<_>>(),
            [Fe32::P, Fe32::Y, Fe32::S, Fe32::J,]
        );
