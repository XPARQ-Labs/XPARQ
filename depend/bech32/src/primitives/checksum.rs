    ...
    ExtField: super::Bech32Field + super::ExtensionField<BaseField = Fe32>,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            f,
            "// Code block generated by Checksum::print_impl polynomial "
        )?;
...
impl<Ck: Checksum> Engine<Ck> {
    ...
    pub fn input_fe(&mut self, e: Fe32) {
        let xn = self
            .residue
            .mul_by_x_then_add(Ck::CHECKSUM_LENGTH, e.into());
        for i in 0..5 {
            if xn & (1 << i) != 0 {
...
impl<'hrp> HrpFe32Iter<'hrp> {
    ...
    pub fn new(hrp: &'hrp Hrp) -> Self {
        ...
        Self {
            high_iter: Some(high_iter),
            low_iter: Some(low_iter),
        }
    }
}
