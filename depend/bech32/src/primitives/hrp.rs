...
impl Hrp {
    ...
    pub fn parse_display<T: core::fmt::Display>(data: T) -> Result<Self, Error> {
        ...
        if byte_formatter.index == 0 {
            ...
            Ok(Self {
                buf: byte_formatter.arr,
                size: byte_formatter.index,
            })
        }
    }
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            TooLong(len) => write!(
                f,
                "hrp is too long, found {} characters, must be <= {}",
                len, MAX_HRP_LEN
            ),
...
mod tests {
    ...
    fn parse_display() {
        ...
        assert_eq!(
            Hrp::parse_display(format_args!("{:084}", 1)),
            Err(Error::TooLong(84)),
        );
    ...
    fn parse_display_iterates_chars() {
        assert_eq!(
            Hrp::parse_display(" ❤").unwrap_err(),
            Error::InvalidAsciiByte(b' ')
        );
        assert_eq!(
            Hrp::parse_display("_❤").unwrap_err(),
            Error::NonAsciiChar('❤')
        );
    }
