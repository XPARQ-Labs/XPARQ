...
impl<const DEG: usize> ops::Mul<Fe32> for Fe32Ext<DEG> {
    type Output = Fe32Ext<DEG>;
    fn mul(self, other: Fe32) -> Self::Output {
        self.mul(&other)
    }
}
    ...
    Self: ExtensionField,
{
    ...
    pub const fn new(inner: [Fe32; DEG]) -> Self {
        Self { inner }
    }
...
impl Bech32Field for Fe1024 {
    ...
    fn _mul(&self, other: &Self) -> Self {
        self.mul_by_elem(other)
    }

    #[inline]
    fn _div(&self, other: &Self) -> Self {
        other.multiplicative_inverse() * self
    }
...
mod tests {
        ...
        for i in 0..32 {
            for j in 0..32 {
                if i != 0 || j != 0 {
                    ...
                    assert_eq!(
                        fe1024.multiplicative_inverse().multiplicative_inverse(),
                        fe1024,
                    );
                }
            }
    ...
    fn display_debug_and_rust_code() {
        ...
        assert!(RustCode1024(Fe1024::new([Fe32::P, Fe32::X]))
            .to_string()
            .contains("Fe1024::new"));
