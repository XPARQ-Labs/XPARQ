...
pub fn decode(s: &str) -> Result<(Hrp, Fe32, Vec<u8>), DecodeError> {
    ...
    Ok((
        segwit.hrp(),
        segwit.witness_version(),
        segwit.byte_iter().collect::<Vec<u8>>(),
    ))
}
...
impl From<InvalidWitnessVersionError> for EncodeError {
    #[inline]
    fn from(e: InvalidWitnessVersionError) -> Self {
...
mod tests {
    ...
    fn encode_v1_matches_known_vectors() {
        ...
        assert_eq!(
            std::str::from_utf8(&writer_default).expect("ascii"),
            v1_lower
        );
    ...
    fn can_not_decode_address_too_long() {
        ...
        assert_eq!(
            decode(address).unwrap_err(),
            DecodeError(SegwitHrpstringError::TooLong(91))
        );
    }
}
