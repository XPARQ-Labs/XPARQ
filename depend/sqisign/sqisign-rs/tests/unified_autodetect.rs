...
fn hd_wire_vectors() -> Option<Vec<(Vec<u8>, Vec<u8>)>> {
    let Ok(contents) = std::fs::read_to_string(HD_VECTORS) else {
        eprintln!("skipping external SQIsignHD vectors; fixture not present");
        return None;
    };
...
fn hd_autodetects_and_verifies() {
    let Some(wires) = hd_wire_vectors() else {
        return;
    };
...
fn dimension_mismatch_rejects() {
    ...
    let Some(mut wires) = hd_wire_vectors() else {
        return;
    };
    let (hd_sig_bytes, hd_pk_bytes) = wires.remove(0);
