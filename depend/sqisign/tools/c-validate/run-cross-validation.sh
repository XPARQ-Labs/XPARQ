...
BUILD_DIR=${SQISIGN_C_BUILD_DIR:-"$C_SOURCE/build-xparq-cross"}
VECTOR_DIR=${SQISIGN_C_VECTOR_DIR:-"$BUILD_DIR/xparq-vectors"}
...
compile_verifier() {
    ...
    cc -O2 -std=c11 \
        ...
        -o "$BUILD_DIR/xparq-c-verify-$level" \
...
for level in lvl1 lvl3 lvl5; do
    ...
    "$BUILD_DIR/xparq-c-verify-$level" "$VECTOR_DIR/$level-valid.bin" valid
    "$BUILD_DIR/xparq-c-verify-$level" "$VECTOR_DIR/$level-invalid.bin" invalid
done

cargo check --release --offline --features sqisign-candidate \
    --manifest-path "$XPARQ_DIR/Cargo.toml"

echo "SQIsign C/Rust cross-validation passed for XPARQ"
