...
impl ExtensionStateSet {
    ...
    pub fn preview_created_state_weight(
        ...
        let extension_id = call.extension_id();
        let empty = Namespace::new();
        let namespace = self.namespaces.get(&extension_id).unwrap_or(&empty);
        let mut preview = NamespacePreview {
            extension_id,
            namespace,
            namespaces: &self.namespaces,
        ...
        registry.apply(context, call, &mut preview)?;
        let created_state_weight =
            preview
                .changes
                .iter()
                .try_fold(0_u64, |total, (key, value)| {
                    match (namespace.contains_key(key), value) {
                        (false, Some(value)) => {
                            let entry = key
                                .len()
...
impl ExtensionStateRead for NamespacePreview<'_> {
    fn get(&self, key: &[u8]) -> Result<Option<Vec<u8>>, ExtensionFailure> {
        validate_key(key)?;
        match self.changes.get(key) {
            Some(value) => Ok(value.clone()),
            None => Ok(self.namespace.get(key).cloned()),
        }
    }

    fn entries(&self) -> Result<Vec<(Vec<u8>, Vec<u8>)>, ExtensionFailure> {
        let mut entries = self.namespace.clone();
    ...
    fn state_size(&self) -> Result<usize, ExtensionFailure> {
        let mut size = namespace_size(self.namespace)?;
        for (key, value) in &self.changes {
            let original = self.namespace.get(key);
            if let Some(original) = original {
                size = size
                    .checked_sub(key.len() + original.len())
                    .ok_or(ExtensionFailure::StateEntryLimit)?;
            }
...
impl ExtensionStateWrite for NamespacePreview<'_> {
    fn put(&mut self, key: Vec<u8>, value: Vec<u8>) -> Result<(), ExtensionFailure> {
        ...
        let current_entries = self
            ...
            .checked_add(
                self.changes
                    .iter()
                    .filter(|(key, value)| !self.namespace.contains_key(*key) && value.is_some())
                    .count(),
            )
            .and_then(|count| {
                count.checked_sub(
                    self.changes
                        .iter()
                        .filter(|(key, value)| self.namespace.contains_key(*key) && value.is_none())
                        .count(),
                )
            })
            .ok_or(ExtensionFailure::StateEntryLimit)?;
        if is_new && current_entries >= EXTENSION_STATE_MAX_ENTRIES {
...
fn namespace_size(namespace: &Namespace) -> Result<usize, ExtensionFailure> {
    namespace.iter().try_fold(0_usize, |total, (key, value)| {
        total
            .checked_add(key.len())
            .and_then(|size| size.checked_add(value.len()))
            .ok_or(ExtensionFailure::StateEntryLimit)
...
mod tests {
    use super::*;
    use crate::common::{Extension, Height};
