...
mod transaction_tests {
    ...
    impl TransactionStateView for FalconAccountState {
        ...
        fn account_public_key(&self, _: Address) -> Option<PublicKey> {
            None
        }
    }

    #[test]
    fn account_authorization_is_active_from_genesis_and_verified() {
        let signing = crypto::SigningSeed::new(crypto::Signature::MlDsa65, [26; 32]);
        let public_key = signing.public_key();
        let sender = crypto::address_from_public_key(&public_key);
        let id = CoinHash::from_bytes([27; CoinHash::SIZE]);
        // One spendable output replaces one consumed UTXO; only the newly
        // revealed key adds ledger state. Transaction history is charged below.
        let ledger_burn = crate::consensus::account_key_state_weight(&public_key).unwrap();
