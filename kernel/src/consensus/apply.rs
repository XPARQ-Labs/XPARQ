...
pub fn apply_genesis<State>(
    state: &mut State,
    block: Block,
    expected_hash: crypto::BlockHash,
) -> Result<(), State::Error>
where
    State: ApplyBlockState,
{
...
fn validate_for_apply(
    ...
    let emission = if block.is_genesis() {
        ...
        let parent_emission = if block.height().0 <= 1 {
            ...
            chain
                .block(&crate::blockchain::Height(block.height().0 - 1))
...
fn expected_difficulty(
    chain: &Chain,
    height: crate::blockchain::BlockHeight,
) -> Result<u32, ConsensusError> {
    if height.0 == 0 {
        return Ok(crate::consensus::GENESIS_DIFFICULTY);
    }
    let parent = chain
        .block(&crate::blockchain::Height(height.0 - 1))
        .ok_or(ConsensusError::InvalidPreviousHash)?;
    expected_difficulty_for_height(height.0, parent.difficulty(), |height| {
        chain
            .block(&crate::blockchain::Height(height))
            .ok_or(ConsensusError::InvalidPreviousHash)?
            .block_weight()
            .try_into()
...
pub fn expected_next_difficulty(chain: &Chain) -> Result<u32, ConsensusError> {
    let height = crate::blockchain::Height(
        chain
            .tip_height()
